...
const ServicesCards: React.FC = () => {
  const services = [
    ...
    },
    {
      ...
      `,
      icon: <FaSearchDollar className="h-10 w-10 sm:h-14 sm:w-14 md:h-18 md:w-18 lg:h-24 lg:w-24 text-black mx-auto mb-4" />,
    },
    {
      ...
      icon: (
        <svg
          ...
          viewBox="0 0 448 512"
          className="h-10 w-10 sm:h-12 sm:w-12 md:h-14 md:w-14 lg:h-18 lg:w-18 text-blue-600 mx-auto mb-4"
          >
    ...
    },
    {
      ...
      icon: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 512 512"
          className="h-12 w-12 sm:h-16 sm:w-16 md:h-20 md:w-20 lg:h-24 lg:w-24 text-blue-600 mx-auto mb-3"
          >
  ...
  return (
    ...
    <section className="w-full bg-white py-24 px-4">
      <div className="max-w-7xl mx-auto px-5 sm:px-10 md:px-12 lg:px-5">
        <h1 className="text-center text-4xl md:text-5xl font-bold text-gray-900 pb-12">
        ...
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 xl:grid-cols-3">
          {services.map((service) => (
            <Link
              key={service.title}
              href={service.href}
              className="group relative block h-60 sm:h-64 lg:h-72 transition-all duration-300"
              aria-label={`Learn more about ${service.title}`}
            >
              <span className="absolute inset-0 border-2 border-dashed border-blue-600 rounded-xl transition-transform transform group-hover:scale-105 group-hover:shadow-xl group-hover:shadow-blue-500"></span>

              <div className="relative flex flex-col items-center justify-center h-full border-2 border-blue-600 bg-white rounded-xl p-6 transition-transform transform group-hover:-translate-x-2 group-hover:-translate-y-2 group-hover:shadow-lg group-hover:shadow-blue-500">
                <div className="text-center">
                  {typeof service.icon === 'string' ? (
                    <Image
                      ...
                      width={80}
                      height={80}
                      className="h-10 w-10 sm:h-14 sm:w-14 md:h-16 md:w-16 lg:h-20 lg:w-20 text-blue-600 mx-auto mb-4"
                    />
                  ) : (
                    service.icon
                  )}
